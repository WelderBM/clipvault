  ...
  return (
    <div className="px-4 pt-4 pb-24">
      <header className="mb-6">
        <h1 className="font-display text-2xl tracking-wider text-teal">
          ESTRATÉGIA FCC
        </h1>
        <p className="font-body text-white/40 text-sm mt-1">
          Hot topics e armadilhas por matéria
        </p>
      </header>
      ...
      <div className="space-y-2">
        {STRATEGY_DISCIPLINES.map((d, idx) => {
          ...
          const strategy = DISCIPLINE_STRATEGIES.find(
            (s) => s.discipline === d,
          );
                            ...
                            <div key={i} className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-2 mb-1">
                                  <span
                                    className={`font-body text-xs leading-snug ${parseInt(t.fccFreq) >= 70 ? "text-white/80" : "text-white/50"}`}
                                  >
                                    {t.topic}
                                  </span>
                                  <span
                                    className={`font-mono text-[10px] flex-shrink-0 ${FREQ_COLOR(t.fccFreq)}`}
                                  >
                                    {t.fccFreq}
                                  </span>
                                </div>
        ...
        {/* TI separado */}
        {(() => {
          const tiStrategy = DISCIPLINE_STRATEGIES.find(
            (s) => s.discipline === "ti",
          );
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="font-body text-sm text-white/90">
                      TI — Conhecimentos Específicos
                    </span>
                    <span className="font-mono text-xs text-white/40 ml-2 flex-shrink-0">
                      40 questões
                    </span>
                  </div>
                  <p className="font-mono text-[10px] text-white/30 mt-0.5">
                    Programador — Código 31
                  </p>
                </div>
                <svg
                  className={`w-4 h-4 text-white/30 flex-shrink-0 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
                          ...
                          {tiStrategy.topTopics.map((t, i) => (
                            <div key={i} className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="h-0.5 rounded-full bg-white/5">
                                  <div
                                    className={`h-0.5 rounded-full ${FREQ_BAR(t.fccFreq)}`}
                                    style={{ width: t.fccFreq }}
                                  />
                                </div>
                              </div>
